...
import type { BackendsMap, DirectoryCapabilities, DirectoryListing, FileEntry, PluginOpeners, PublicConfig, Session } from "./types";
...
export const pluginApi = {
  list: (signal?: AbortSignal) => api.get<PluginOpeners>("/api/plugin", { signal }),
};
...
export const adminApi = {
  ...
  getConfig: (signal?: AbortSignal, setupToken?: string) =>
    api.get<Record<string, Record<string, unknown>>>("/admin/api/config", { signal, headers: setupToken ? { "X-Filestash-Setup-Token": setupToken } : undefined }),
  /** POST /admin/api/config — save the full config tree. */
  saveConfig: (config: unknown, setupToken?: string) => api.post<null>("/admin/api/config", config as Record<string, unknown>, { headers: setupToken ? { "X-Filestash-Setup-Token": setupToken } : undefined }),
...
export const filesApi = {
  /** GET /api/files/ls?path=&cursor=&limit= — list one directory page. */
  ls: async (path: string, cursor?: string, signal?: AbortSignal): Promise<DirectoryListing> => {
    const envelope = await api.getEnvelope<FileEntry[]>("/api/files/ls", {
      query: { path, cursor, limit: 250 },
      signal,
    });
    return {
      entries: envelope.results ?? [],
      permissions: (envelope.permissions ?? {}) as DirectoryCapabilities,
      nextCursor: envelope.headers.get("X-Next-Cursor") || undefined,
    };
  },
  ...
  catUrl: (path: string) => buildUrl("/api/files/cat", { path }),
  /** POST /api/files/cat?path= — save edited text content. */
  save: async (path: string, content: string) => {
    const res = await fetch(buildUrl("/api/files/cat", { path }), {
      method: "POST",
      credentials: "include",
      headers: { "X-Requested-With": "XmlHttpRequest", "Content-Type": "text/plain" },
      body: content,
    });
    if (!res.ok) throw new ApiRequestError((await res.text()) || `save failed (${res.status})`, res.status);
  },
  ...
  upload: async (path: string, body: Blob | File) => {
    ...
    if (!res.ok) throw new ApiRequestError((await res.text()) || `upload failed (${res.status})`, res.status);
  },
